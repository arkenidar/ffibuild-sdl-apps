...
_G=setmetatable(_G, {
    ...
    if "SDL_"==string.sub(index,1,4) then
      local substring = string.sub(index,5,-1)
      return SDL.e[substring] or SDL[substring]
    end
	end
})
...
local rect = ffi.new('struct SDL_Rect')
rect.x = xywh[1]
rect.y = xywh[2]
rect.w = xywh[3]
rect.h = xywh[4]
...
function key_down(key)
  return ( event.type == SDL_KEYDOWN and event.key.keysym.scancode == SDL.e["SCANCODE_"..string.upper(key)] )
end
